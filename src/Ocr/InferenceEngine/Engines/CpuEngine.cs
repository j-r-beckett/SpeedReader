...
public class CpuEngine : IInferenceEngine
{
    ...
    private readonly List<InferenceThread> _allThreads = [];
    ...
    {
        var cores = config.Cores.Count == 0 ? [0] : config.Cores;
        foreach (var core in cores)
        {
            var thread = new InferenceThread(inferenceKernel, core, t => _threads.Add(t));
    ...
    public int CurrentMaxCapacity() => _allThreads.Count;
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        await Task.WhenAll(_allThreads.Select(thread => Task.Run(thread.Dispose)));

        _activatorCts.Dispose();
        _jobs.Dispose();
        _threads.Dispose();
    ...
    private class InferenceThread : IDisposable
    {
        ...
        public InferenceThread(IInferenceKernel inferenceKernel, int core, Action<InferenceThread> requeueThread)
        {
            _core = core;
