...
public class DBNetBlock
{
    ...
    public DBNetBlock(InferenceSession session, DbNetConfiguration config, Meter meter)
    {
        var preprocessingBlock = new DBNetPreprocessingBlock(config);
        var modelRunnerBlock = new DBNetModelRunnerBlock(session, config, meter);
        var postprocessingBlock = new DBNetPostprocessingBlock(config);
