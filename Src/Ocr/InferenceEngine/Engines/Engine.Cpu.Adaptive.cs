...
public class AdaptiveCpuEngine : IInferenceEngine
{
    ...
    {
        // TODO
        // Constructor accepts a nullable IMetricRecorder, but use `GetRequiredKeyedService` anyway so that if we're
        // missing one in DI we get an error right away
        var metricRecorder = serviceProvider.GetKeyedService<IMetricRecorder<IInferenceEngine>>(key);
        ...
        return new AdaptiveCpuEngine(options, kernel, metricRecorder);
    }

    private AdaptiveCpuEngine(AdaptiveCpuEngineOptions options, IInferenceKernel inferenceKernel, IMetricRecorder<IInferenceEngine>? metricRecorder = null)
    {
        _managedExecutor = new ManagedExecutor(1);
    ...
    public async Task Tune(CancellationToken stoppingToken)
    {
        ...
        //     if (statistics.AvgParallelism < _managedExecutor.PoolSize - 2)
        //     {
        ...
        // {
        //     _managedExecutor.IncreasePoolSize(1);
        //     lastAction = ActionType.Increase;
        // }
        //
        // void DecrementParallelism()
        // {
        //     if (_managedExecutor.PoolSize > 1)
        //     {
        //         _managedExecutor.IncreasePoolSize(-1);
