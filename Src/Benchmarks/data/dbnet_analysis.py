...
def _(alt, data, mo, pl):
    def _():
        ...
        # Fit model
        threads_data = data['threads'].to_numpy()
        throughput_values = data['throughput'].to_numpy()
        (L_fit_tp, k_fit_tp, x0_fit_tp), _ = curve_fit(logistic, threads_data, throughput_values, p0=[25, 1, 3])

        # Calculate R^2
        predictions_tp = logistic(threads_data, L_fit_tp, k_fit_tp, x0_fit_tp)
        r_squared_tp = 1 - np.sum((throughput_values - predictions_tp)**2) / np.sum((throughput_values - throughput_values.mean())**2)

        # Create smooth prediction curve
        threads_smooth = np.linspace(1, 12, 100)
        pred_tp_df = pl.DataFrame({
            'threads': threads_smooth,
            'predicted_throughput': logistic(threads_smooth, L_fit_tp, k_fit_tp, x0_fit_tp)
        })

        # Visualization
        observed_throughput = alt.Chart(data).mark_circle(size=100, color='steelblue').encode(
            x=alt.X('threads:Q', title='Threads'),
            y=alt.Y('throughput:Q', title='Throughput (tiles/sec)'),
            tooltip=['threads:Q', 'throughput:Q']
        )

        fitted_throughput = alt.Chart(pred_tp_df).mark_line(color='orange', strokeWidth=2).encode(
            x='threads:Q',
            y='predicted_throughput:Q'
        )

        throughput_logistic_chart = (observed_throughput + fitted_throughput).properties(
            width=600, height=400, title='Thread Count vs Throughput (Direct Logistic)'
        )

        # Summary
        throughput_logistic_summary = mo.md(f"""
        ## Thread Count -> Throughput (Direct Logistic)

        **Logistic Best Fit**

        `throughput = {L_fit_tp:.3f} / (1 + exp(-{k_fit_tp:.3f} * (threads - {x0_fit_tp:.3f})))`

        - `L` = {L_fit_tp:.3f} (asymptote, predicted maximum throughput)
        - `k` = {k_fit_tp:.3f} (steepness)
        - `x_0` = {x0_fit_tp:.3f} (point of maximum growth rate)
        ...
        **Correctness**
        ...
        # Return visualization and parameters
        return mo.vstack([throughput_logistic_chart, throughput_logistic_summary]), (L_fit_tp, k_fit_tp, x0_fit_tp)

    throughput_logistic_viz, throughput_logistic_params = _()
    throughput_logistic_viz
    return
